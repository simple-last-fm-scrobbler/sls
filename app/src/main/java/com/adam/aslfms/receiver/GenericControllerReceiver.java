    ...
    private void parseData(Context ctx, String action, Bundle bundle){
        ...
        try {
            if (action != null) {
                ...
                try {
                    if (bundle.containsKey("player")) {
                        ...
                        if (playerPackage != null && !playerPackage.isEmpty()) {
                            ...
                            mMusicApi = MusicAPI.fromReceiver(ctx, playerName, playerPackage, "genericrcvr", false);
                ...
                if (mMusicApi == null) {
                    mMusicApi = MusicAPI.fromReceiver(ctx, ctx.getResources().getString(R.string.notification_controller), ctx.getPackageName(), "genericrcvr", false);
                    setMusicAPI(mMusicApi);
                }
